...
async function fetchTakenDates(cabinId: string, from: string, to: string): Promise<string[]> {
  ...
  return Array.isArray(json.dates)
  ...
  head: () => ({
    meta: [
      { title: "Book a Cabin | Rajawali D'Cabin Chalet, Kuala Terengganu" },
      {
        name: "description",
        content:
          "Reserve a private cabin at Rajawali D'Cabin Chalet in Chendering, Kuala Terengganu. Easy online booking with a refundable RM50 security deposit per room.",
      },
      ...
      { property: "og:title", content: "Book a Cabin — Rajawali D'Cabin Chalet, Kuala Terengganu" },
      {
        ...
        content:
          "Reserve a private riverside cabin in Chendering, Kuala Terengganu. Refundable RM50 security deposit per room.",
      },
      { property: "og:url", content: "https://drajawalicabin.com/book" },
    ],
    links: [{ rel: "canonical", href: "https://drajawalicabin.com/book" }],
  }),
  component: BookPage,
});
...
function BookPage() {
  ...
  const [priceByType, setPriceByType] = useState<
    Record<string, { nights: number; subtotal: number; total: number; perNight: number }>
  >({});
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setRemembered({
          ...saved,
          reference: (data as { reference?: string }).reference ?? saved.reference,
        });
      } catch {
  ...
  useEffect(() => {
    (async () => {
      ...
      const list = ((data ?? []) as CabinRateRow[]).map((c) => ({
        ...c,
        weekday_rate: Number(
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const byType: Record<
          string,
          { nights: number; subtotal: number; total: number; perNight: number }
        > = {};
        const breakdownByType = new Map<
          string,
          Awaited<ReturnType<typeof previewPriceDetailed>>["nights"]
        >();
        ...
        <div className="mx-auto mt-6 max-w-3xl px-6 lg:px-10">
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-forest/30 bg-coconut px-5 py-4">
            <p className="text-sm text-foreground/85">
              ...
              <span className="font-mono text-forest">{remembered.reference || "—"}</span>. Need to
              send a receipt? Open it instead of booking again.
            </p>
            <Link
              to="/manage-booking"
...
function DetailsStep(props: {
  ...
  guests: string;
  setGuests: (s: string) => void;
  kids: string;
  setKids: (s: string) => void;
  comforter: boolean;
  setComforter: (b: boolean) => void;
  name: string;
  ...
  icNumber: string;
  setIcNumber: (s: string) => void;
  relationship: string;
  setRelationship: (s: string) => void;
  vehicleType: string;
  setVehicleType: (s: string) => void;
  vehicleNumber: string;
  setVehicleNumber: (s: string) => void;
  notes: string;
  ...
  return (
    <section className="mx-auto grid max-w-6xl gap-12 px-6 py-16 lg:grid-cols-[1.2fr_1fr] lg:gap-16 lg:px-10 lg:py-20">
      <form onSubmit={submit} className="order-1 lg:order-1">
        ...
        {isAnyCabin && (
          <div className="mb-4 rounded-xl border border-forest/30 bg-forest/[0.04] p-3">
            <div className="flex items-center justify-between gap-2">
              <p className="text-[10px] uppercase tracking-[0.25em] text-forest">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        <span className="text-[9px] uppercase tracking-widest text-forest underline">
                          Select
                        </span>
                      </div>
                    </button>
          ...
          {(() => {
            ...
            return (
              <div className="mt-3 rounded-lg border border-red-300 bg-red-50 px-3 py-2 text-[12px] text-red-800">
                ...
                {hits.length > 1
                  ? ` (+${hits.length - 1} more night${hits.length - 1 === 1 ? "" : "s"})`
                  : ""}{" "}
                — {reason}. Please choose different dates.
              </div>
            );
          })()}
          <div className="mt-3">
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 hidden text-[9px] text-stone/80 sm:block">
                        Weekday RM {sample?.weekday_rate} · Weekend RM {sample?.weekend_rate} ·
                        School holiday RM {sample?.school_holiday_rate}
                      </p>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <button
                      ...
                      <span className="min-w-[1.25rem] text-center text-sm font-medium text-forest">
                        {it.qty}
                      </span>
                      <button
                        type="button"
                  ...
                  {tooMany && (
                    <p className="mt-1.5 text-[11px] text-red-700">
                      Only {free} {g.label} room{free === 1 ? "" : "s"} free for these dates —
                      reduce qty or change dates.
                    </p>
                  )}
                </li>
              );
        ...
        <p className="mt-8 mb-3 text-[11px] uppercase tracking-[0.3em] text-stone">
          {bt.step2Eyebrow}
        </p>
        <div className="grid gap-px overflow-hidden rounded-2xl border border-border bg-border sm:grid-cols-2">
          ...
          <Field
            ...
            type="text"
            value={icNumber}
            onChange={(v) => setIcNumber(formatIcNumber(v))}
            placeholder={bt.g.icNumberPh}
          />
          <Field
            label={bt.g.vehicleType}
            type="text"
            value={vehicleType}
            onChange={(v) => setVehicleType(formatVehicleType(v))}
            placeholder={bt.g.vehicleTypePh}
          />
          <Field
            label={bt.g.vehicleNumber}
            type="text"
            value={vehicleNumber}
            onChange={(v) => setVehicleNumber(formatVehicleNumber(v))}
            placeholder={bt.g.vehicleNumberPh}
          />
          <Field
            label={bt.g.relationship}
            type="text"
            value={relationship}
            onChange={setRelationship}
            placeholder={bt.g.relationshipPh}
            full
          />
          <TextArea
            label={bt.g.notes}
            value={notes}
            onChange={setNotes}
            placeholder={bt.g.notesPh}
          />
        </div>

        <div className="mt-6 rounded-xl border border-dashed border-border bg-coconut px-5 py-4 text-sm text-stone">
          <p>
            <strong>{bt.info.roomNum}</strong> {bt.info.roomNumNote}
          </p>
          <p className="mt-1">
            <strong>{bt.info.bookingNum}</strong> {bt.info.bookingNumNote}
          </p>
        </div>
        ...
        <div className="mt-8 rounded-2xl border border-border bg-card p-5">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <label
              className={`cursor-pointer rounded-xl border p-4 transition ${paymentType === "deposit" ? "border-forest bg-forest/[0.04]" : "border-border bg-card hover:border-forest/40"}`}
            >
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-medium text-forest">
                    Reserve with RM{SECURITY_DEPOSIT_PER_ROOM} security deposit/room
                  </p>
                  <p className="mt-1 text-xs text-stone">
                    RM{SECURITY_DEPOSIT_PER_ROOM} per room secures your dates. This is a refundable
                    security deposit, not part of the room rate. Full room rate is due 7 days before
                    check-in.
                  </p>
                </div>
              </div>
            ...
            <label
              className={`cursor-pointer rounded-xl border p-4 transition ${paymentType === "full" ? "border-forest bg-forest/[0.04]" : "border-border bg-card hover:border-forest/40"}`}
            >
              <div className="flex items-start gap-3">
                <input
                  type="radio"
                ...
                <div>
                  <p className="font-medium text-forest">
                    Pay in full now
                    {price
                      ? ` (RM ${(price.total - discounts.reduce((s, d) => s + d.amount, 0) + totalRooms * SECURITY_DEPOSIT_PER_ROOM).toFixed(2)})`
                      : ""}
                  </p>
                  <p className="mt-1 text-xs text-stone">
                    Settle the full room rate plus a refundable RM{SECURITY_DEPOSIT_PER_ROOM}/room
        ...
        <div className="mt-8 rounded-2xl border border-border bg-card p-5">
          ...
          {paymentType === "deposit" ? (
            <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-foreground/80">
              <li>
                RM {SECURITY_DEPOSIT_PER_ROOM} per room is a refundable booking/security deposit to
                secure your dates.
              </li>
              <li>
                This deposit is not part of the room rate and will be refunded after check-out,
                subject to room inspection.
              </li>
              <li>The full room rate is due 7 days before check-in; we'll email a reminder.</li>
              <li>
                Up to 2 children under 12 years old stay free per room (sharing existing bedding).
              </li>
              <li>
                <strong>Check-out is by 12:00 PM.</strong> Late check-out is charged at{" "}
                <strong>RM10 per hour</strong> (rounded up) and deducted from your security deposit.
              </li>
            ...
            <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-foreground/80">
              <li>
                Pay the full room rate plus a refundable RM {SECURITY_DEPOSIT_PER_ROOM}/room
                security deposit now.
              </li>
              <li>
                The security deposit is not part of the room rate and will be refunded after
                check-out, subject to room inspection.
              </li>
          ...
          <label className="mt-4 flex items-start gap-3">
            ...
            <span className="text-sm leading-relaxed text-foreground/85">
              {paymentType === "deposit"
                ? "I agree to the property rules (RM " +
                  SECURITY_DEPOSIT_PER_ROOM +
                  "/room refundable security deposit to secure dates, full room rate due 7 days before check-in, 7-day cancellation policy)."
                : "I agree to the property rules (full room rate + refundable RM " +
                  SECURITY_DEPOSIT_PER_ROOM +
                  "/room security deposit now, locker code on verification, 7-day cancellation policy)."}
            </span>
          </label>
        </div>
...
function PaymentStep({
  ...
  const [remaining, setRemaining] = useState(
    Math.max(0, new Date(booking.holdExpiresAt).getTime() - Date.now()),
  );
  useEffect(() => {
    const t = setInterval(
      () => setRemaining(Math.max(0, new Date(booking.holdExpiresAt).getTime() - Date.now())),
      1000,
    );
        ...
        <div className="flex items-baseline justify-between">
          <div>
            <p className="text-xs uppercase tracking-widest text-stone">
              {paymentType === "full"
                ? "Room rate + security deposit due now"
                : "Security deposit due now"}
            </p>
            <p className="font-display text-4xl text-forest">
              RM{" "}
              {paymentType === "full"
                ? (booking.total + booking.securityDeposit).toFixed(2)
            ...
            {paymentType === "full" && (
              <p className="mt-1 text-xs text-stone">
                Includes refundable RM {booking.securityDeposit.toFixed(2)} security deposit —
                locker code on confirmation
              </p>
            )}
          </div>
      ...
      {cardEnabled && (
        <div className="mt-6 rounded-2xl border border-forest/30 bg-coconut p-6">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[11px] uppercase tracking-[0.25em] text-forest">
                Fastest — auto-confirm
              </p>
          ...
          <dl className="mt-4 space-y-2 text-sm">
            <div className="flex items-center justify-between gap-3">
              ...
              <dd className="flex items-center gap-2">
                ...
                <button
                  onClick={() => copy(acctNum)}
                  className="text-[10px] uppercase tracking-widest text-forest hover:underline"
                >
                  {bt.pay.copy}
                </button>
              </dd>
  ...
  return (
    <section className="mx-auto max-w-2xl px-6 py-24 text-center lg:px-10">
      ...
      <p className="mt-6 text-foreground/75">
        {bt.done.body1} <span className="font-mono text-forest">{reference}</span>
        {bt.done.body2} <span className="text-forest">{email}</span>
        {bt.done.bodyTail}
      </p>
      ...
      <p className="mt-6 text-xs text-stone">
        ...
        <Link to="/find-booking" className="underline hover:text-forest">
          Find Booking
        </Link>{" "}
...
type StaySummaryCardProps = {
  previewCabin?: Cabin;
  cart: CartItem[];
  groupByType: Map<string, CabinGroup>;
  totalRooms: number;
  checkin: string;
  checkout: string;
  guests: string;
...
function StaySummaryCard(props: StaySummaryCardProps) {
  const { t } = useLanguage();
  const bt = t.book;
  const {
    previewCabin,
    ...
    <div className="overflow-hidden rounded-2xl border border-border bg-card">
      ...
      <div className="p-6">
        ...
        <dl className="mt-6 divide-y divide-border text-sm">
          ...
          <Row
            label={bt.summary.guests}
            value={`${guests} adult${Number(guests) === 1 ? "" : "s"}${Number(kids) > 0 ? ` + ${kids} child${Number(kids) === 1 ? "" : "ren"} <12` : ""}`}
          />
          ...
          {price && (
            <>
              ...
              {price.comforter_total > 0 && (
                <Row
                  label={bt.summary.comforterLabel}
                  value={`RM ${price.comforter_total.toFixed(2)}`}
                />
              )}
                  ...
                  <>
                    ...
                    {!couponError && couponRow && couponAmount === 0 && (
                      <p className="mt-0.5 text-[10px] leading-tight text-amber-700">
                        Code accepted but doesn't apply to this stay (check minimum nights or
                        dates).
                      </p>
                    )}
                  </>
              ...
              <Row
                label={bt.summary.securityDeposit}
                value={`RM ${(totalRooms * SECURITY_DEPOSIT_PER_ROOM).toFixed(2)}`}
              />
            </>
          )}
        </dl>
          ...
          <div className="mt-4 flex items-baseline justify-between rounded-xl bg-coconut px-4 py-3">
            ...
            <span className="font-display text-2xl text-forest">
              ...
              {(
                ...
                totalRooms * SECURITY_DEPOSIT_PER_ROOM
              ).toFixed(2)}
            </span>
          </div>
        )}
    ...
    <header className="border-b border-border bg-coconut">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5 lg:px-10">
        ...
        <div className="flex items-center gap-4">
          ...
          <BookInWhatsAppLink className="text-xs uppercase tracking-widest text-stone hover:text-forest">
            Book in WhatsApp
          </BookInWhatsAppLink>
...
function Field({
  ...
  return (
    <label
      className={`flex flex-col gap-1 bg-card px-5 py-4 text-left ${full ? "sm:col-span-2" : ""}`}
    >
      ...
      >
        ...
        {cabins.map((c) => (
          <option key={c.id} value={c.id}>
            {optionTpl.replace("{name}", c.name).replace("{cap}", String(c.capacity))}
          </option>
        ))}
      </select>
    </label>
