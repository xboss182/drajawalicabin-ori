...
const Email = (p: Props) => {
  ...
  return (
    <EmailLayout preview={`New reservation ${ref} — ${isFull ? 'paid in full' : 'security deposit received'}`}>
      <Heading as="h2" style={styles.h2}>New reservation received</Heading>
      <Text style={styles.p}>
        {p.guestName ?? 'A guest'} just submitted a booking with {isFull ? 'full room rate + security deposit' : 'security deposit'} proof. Please review and confirm.
      </Text>
      <div style={styles.panel}>
        ...
        {isFull ? (
          <>
            <p style={styles.row}><span style={styles.label}>Security deposit</span><span style={styles.value}>{money(p.securityDeposit)} (refundable)</span></p>
          ...
          <>
            <p style={styles.row}><span style={styles.label}>Security deposit</span><span style={styles.value}>{money(p.securityDeposit)} (refundable)</span></p>
            <p style={styles.row}><span style={styles.label}>Room rate balance</span><span style={styles.value}>{money(p.remaining)}</span></p>
          </>
        )}
      </div>
...
export const template = {
  ...
  previewData: { guestName: 'Ahmad', guestEmail: 'ahmad@example.com', guestPhone: '+60123456789', reference: 'RJW-1234', bookingId: 'abc-123', roomType: '2 PAX Family Suite', checkIn: '2026-07-10', checkOut: '2026-07-12', nights: 2, guests: 2, total: 480, securityDeposit: 50, remaining: 480, paymentType: 'deposit' } as Props,
