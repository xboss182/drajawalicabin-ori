...
The booking bridge (MNC-961) calls this API over loopback; browsers never reach
it.
...
| `/opt/waha/.env` | `WAHA_API_KEY`, `WAHA_WEBHOOK_HMAC_KEY`, `WA_MACHINE_SECRET`, `MACHINE_URL`, `WA_MACHINE_KEY_ID` (mode 600, never committed) |
...
| `/opt/waha/bridge.Dockerfile` | copied into `/opt/waha/wa-bridge-src/Dockerfile` at deploy time (node:22-alpine, zero deps) |
| `/opt/waha/wa-bridge-src/` | deploy copy of reviewed `main`'s `wa-bridge/` tree; refresh before an intentional bridge rollout |
...
The `bridge` service builds from `/opt/waha/wa-bridge-src/`, an explicit deploy
copy of reviewed `main`'s `wa-bridge/` tree. It joins `waha-net`, binds loopback
`8180`, and `depends_on` a healthy `waha`. Before an intentional bridge rollout:

```bash
cd /opt/waha
rsync -a --delete <repo>/wa-bridge/ wa-bridge-src/
cp bridge.Dockerfile wa-bridge-src/Dockerfile
docker compose up -d --build bridge
```

The WAHA session webhook is configured (PUT `/api/sessions/default`) with
`url=http://wa-bridge:8080/webhook/waha`, `events=[message, session.status]`,
`hmac.key = WAHA_WEBHOOK_HMAC_KEY`. Verified 2026-08-22: valid signature → 200,
bad signature → 401; webhook config survives container restart.

`healthcheck.sh` exit codes (dependency-aware — WAHA checked before bridge):

| Exit | Meaning |
|---|---|
| 0 | WAHA `WORKING` and bridge `/health` ok |
