...
const MIGRATION = join(
  ...
  "supabase",
  "migrations",
  "20260822_wa_booking_bridge.sql",
);
const ADMIN_MIGRATION = join(
  ...
  "supabase",
  "migrations",
  "20260822_wa_admin_operations.sql",
);
...
describe("wa sql invariants (sequential)", () => {
  ...
  test("migration applies cleanly on the slab", { skip: !hasDocker }, () => {
    const tables = psql(
      "select count(*) from information_schema.tables where table_schema='public' and table_name in ('wa_conversations','wa_events','wa_outbox','wa_proofs','wa_nonces','wa_runtime_status','wa_admin_audit')",
    );
    assert.equal(tables, "7");
  });

  test(
    "TWO CONCURRENT CLAIMS cannot reserve the same cabin nights",
    { skip: !hasDocker, timeout: 120000 },
    async () => {
      const claim = (chat) => `
    select count(*) from wa_claim_hold(
      ...
      assert.equal(
        winners,
        1,
        `expected exactly one winning claim; got A=${JSON.stringify(a)} B=${JSON.stringify(b)}`,
      );
      ...
      RACE_WINNER_CHAT = psql(
        "select wa_chat_id from booking_requests where source='wa' and status='pending_payment'",
      );
  ...
  test(
    "outbox claim/restart semantics: retries capped at 3 then failed",
    { skip: !hasDocker },
    () => {
      ...
      assert.equal(
        ...
        "0",
        "fresh sending row must not be re-claimed",
      );
